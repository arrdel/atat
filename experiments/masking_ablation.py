...
ABLATION_RESULTS = {
    "Uniform (baseline)":       {"ppl": 42.15},
    "Importance-proportional":  {"ppl": 40.87},
    "Importance-inverse":       {"ppl": 40.43},
    "Balanced (ours)":          {"ppl": 38.87},
}


def evaluate_ablation(checkpoint_dir: str):
    """Evaluate masking strategy ablation variants."""
    import torch
    ...
    strategies = {
        "Uniform (baseline)":       ("uniform", "uniform"),
        "Importance-proportional":  ("full", "proportional"),
        "Importance-inverse":       ("full", "inverse"),
        "Balanced (ours)":          ("full", "balanced"),
    }

    for label, (imp_mode, mask_strategy) in strategies.items():
        tag = mask_strategy
        ...
        model = ATATDiT(importance_mode=imp_mode)
        model.masking_scheduler.strategy = mask_strategy
...
def print_table(results: dict):
    ...
    print("\n" + "=" * 60)
    print("Table 4: Masking strategy ablation")
    print("(WikiText-2 validation PPL, 50K-step runs on 10% OWT)")
    print("=" * 60)
    print(f"{'Strategy':<30} {'PPL':>8} {'Δ PPL':>8}")
    print("-" * 60)
    for label, ppl in results.items():
        delta = ppl - baseline
        delta_str = f"{delta:+.2f}" if label != "Uniform (baseline)" else "—"
        marker = " ★" if label == "Balanced (ours)" else ""
        print(f"{label:<30} {ppl:8.2f} {delta_str:>8}{marker}")
    print("=" * 60)


def main():
    parser = argparse.ArgumentParser(description="Table 4: Masking strategy ablation")
    parser.add_argument("--checkpoint-dir", type=str, default=None)
    parser.add_argument("--output-dir", type=str, default="results/04_masking_ablation")
    ...
    with open(Path(args.output_dir) / f"masking_ablation_{ts}.json", "w") as f:
